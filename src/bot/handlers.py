...
async def wait_btn(message: Message, state: FSMContext):
    if message.text.lower() == "мировые кухни":
        ...
        text = 'Что бы вы хотели спросить по мировым кухням? Спросите в формате `хочу есть` или конкрестнее [прием пищи | погода | настроение | вкус]\nИИ ответит только на вопросы по еде. Для завершения напишите "Пока", "Хватит", "Стоп"'
        ...
        text = 'Что бы вы хотели спросить по выбору блюд? Спросите в формате `[русская] кухня` \n ИИ ответит только на вопросы по еде. Для завершения напишите "Пока", "Хватит", "Стоп"'
    else:
